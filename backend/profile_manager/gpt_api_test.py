...
from db_input_processor import ProfileCreator
from tqdm import tqdm  # 진행 상태를 표시하는 라이브러리
import json  # JSON 데이터를 처리하는 모듈

def process_all_resumes(start_index=0, batch_size=1):
    """
    모든 이력서를 처리하는 함수입니다.
    ...
    for resume_file in tqdm(resume_files, desc="이력서 처리 중"):  # 이력서 파일을 tqdm으로 처리합니다.
        ...
        try:
            ...
            profile_creator = ProfileCreator(gpt_result)
...
if __name__ == "__main__":
    ...
    # 또는 특정 범위만 처리
    result = process_all_resumes(start_index=0, batch_size=0)  # 처음 1개만 처리
    print(result)
